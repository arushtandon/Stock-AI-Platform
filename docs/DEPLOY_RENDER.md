...
- **Worker "Ran out of memory (used over 512MB)":** The Starter plan has 512MB RAM. The Blueprint sets `--concurrency=1` so only one Celery process runs. If it still OOMs, upgrade the worker: **stock-ai-worker** → **Settings** → **Instance type** → **Standard** (or higher), then redeploy.
